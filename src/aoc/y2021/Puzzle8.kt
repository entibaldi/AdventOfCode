...
class Puzzle8 : Puzzle(2021, 8) {
    ...
    private val digits = mapOf(0 to "abcefg", 1 to "cf", 2 to "acdeg", 3 to "acdfg", 4 to "bcdf", 5 to "abdfg",
        6 to "abdefg", 7 to "acf", 8 to "abcdefg", 9 to "abcdfg").mapValues { (_, v) -> v.toCharArray().toSet() }
    ...
    private fun part2(input: List<InputLine>): Int = input.sumOf { line ->
        ...
        while (pairMaps.values.any { it.size > 1 }) {
            pairMaps.entries.stream().filter { it.value.size > 1 }.forEach { (chars, possibleValues) ->
                pairMaps.entries.stream().filter { it.value.size == 1 }.forEach { (otherChars, valueSingle) ->
